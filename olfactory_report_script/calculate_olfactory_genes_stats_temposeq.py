...
def calculate_stats_per_col(df, gene_list):
    ...
    col_of_interest = 'Geneid' if 'Geneid' in df.columns else 'Unnamed: 0'
    
    # Filter df by gene list
    df_filtered = df.loc[df[col_of_interest].isin(gene_list)]
    df_filtered.index = df_filtered[col_of_interest]
    df_filtered.index.name = None
    df_filtered.drop(columns=[col_of_interest], inplace=True)
    ...
    for sample,raw_count in series_sum.items():
        ...
        number_expressing_probes = len(expressing_probes)
        expressing_probes_col_name = f'Number of expressing probes (out of {len(gene_list)})'
        ...
        dictionary[sample] = {'Counts in target probes':raw_count, 'Reads in sample':total_count_for_sample, 'Cumulative CPM':cpm, expressing_probes_col_name:number_expressing_probes, 'Average CPM of expressing probes':avg_cpm, 'Probes with count 1': probes_count_1, 'Probes with count 2':probes_count_2, 'Probes with count >=5': probes_count_more_5, 'Probes with count >=10':probes_count_more_10}
    ...
    res = res.T
    # Make fixed name Sample
    res['Sample'] = res.index
    res.reset_index(drop=True, inplace=True)
    #Move sample in front
    cols = list(res.columns)
    print('Pre MODIFIED COLS', cols)

    cols.remove('Sample')
    cols.insert(0, 'Sample')

    print('MODIFIED COLS', cols)
...
def parse_gene_list(file, filename):
    ...
    if filename.endswith('csv'):
        file_content = StringIO(file.decode('utf-8-sig')).readlines()
        file_string = ''.join(file_content)
        df = pd.read_csv(StringIO(file_string), header=None)
        l = list(df[df.columns[0]])
    elif filename.endswith('xlsx'):
        df = pd.read_excel(BytesIO(file), header=None, engine='openpyxl')
        l = list(df[df.columns[0]])
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2:
        ...
        # print()
        res = calculate_stats_per_col(df, gene_list)
        res.to_csv('local_test.csv', index=False)
    else:
        ...
        res = calculate_stats_per_col(df, gene_list)
        print(res)
        res[0].to_csv('local_test.csv', index=False)
